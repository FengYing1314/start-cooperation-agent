...
Inbound `reviewer_fix` means Manager received the Manager copy of a direct R1 -> D1 blocking fix handoff. Record it as the review result. When `Next handoff sent:` starts with `yes`, run the returned follow-up commands in order. When it starts with `no`, use the returned `unsent_handoff`: read `payload_file`, send its exact contents to D1, and only then run `after_send_status_commands`. Do not record `fix_required` or `developer_fix_running` until the exact fix payload is really sent or relayed to D1.
