...
Inbound `reviewer_fix` means Manager received the Manager copy of a direct R1 -> D1 blocking fix handoff. Record it as the review result. When `Next handoff sent:` starts with `yes`, run the returned follow-up commands in order. When it starts with `no`, use the returned `unsent_handoff`: read `payload_file`, send its exact contents to D1, optionally run `after_send_evidence_command` when a tool receipt is available, and only then run `after_send_status_commands`. If that send fails, run `after_send_failed_command` with the concrete error and keep the run status unchanged. Do not record `fix_required` or `developer_fix_running` until the exact fix payload is really sent or relayed to D1.
