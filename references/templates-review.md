...
Use these for Manager review requests, Reviewer decisions, and Developer fix-completion handoffs. These payloads assume direct `codex-thread` mode unless a section says otherwise. Prepare outbound review requests and reviewer fix handoffs with `scripts/prepare_outbound_handoff.py`; validate received or manual-relay payloads with `scripts/validate_handoff.py` when practical.
