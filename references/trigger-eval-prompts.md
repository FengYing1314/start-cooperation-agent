...
Use these prompts for forward-testing whether `start-work` triggers when it should and stays idle for adjacent work. Prepare an isolated fixture with `scripts/prepare_trigger_eval_workspace.py --output-dir <tmp-dir> --print-json`, then prefer the returned `next_commands` and `next_actions` for cli_check, dry-run, run, and score steps. The fixture prep resolves `codex` through PATH, `CODEX_BIN`, or common Windows install paths; pass `--codex-bin <path>` when a specific CLI is required. Preparing a fixture cleans stale artifacts by default; use `--keep-artifacts` only when intentionally preserving traces. The generated plan can also be run with `scripts/run_trigger_eval_plan.py --plan <tmp-dir>/trigger-eval-plan.json --print-json` and scored with `scripts/score_trigger_evals.py --plan <tmp-dir>/trigger-eval-plan.json --print-json`. Run score only after eval artifacts have been written; do not run eval and score in parallel. Use runner and scorer `--id <eval-id>` for focused reruns and `--timeout-seconds <seconds>` for bounded real evals. Empty artifacts, runner errors, and timeouts are inconclusive failures, not evidence that the skill stayed idle. Keep this list small; add real misses as they appear.
